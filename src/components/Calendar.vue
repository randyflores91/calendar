...
export default {
  ...
  methods: {
    async getEvents() {
      let snapshot = await database.collection("calEvent").get();
      let events = [];
    ...
    async addEvent() {
      if (this.name && this.start && this.end) {
        await database.collection('calEvent').add({
          name: this.name,
    ...
    async updateEvent(ev) {
      await database.collection('calEvent').doc(this.currentlyEditing).update({
        details: ev.details
      });
    ...
    async deleteEvent(ev) {
      await database.collection('calEvent').doc(ev).delete();
      this.selectOpen = false;
      this.getEvents();
    },
    viewDay({ date }) {
